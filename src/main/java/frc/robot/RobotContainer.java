...
import frc.robot.commands.drive.DriveFair;
...
import frc.robot.commands.shooter.ShootFair;
...
public class RobotContainer {
  ...
  private final Joystick fairBoard = new Joystick(1);
  ...
  private void configureButtonBindings() {
    ...
    JoystickButton fairShootButton = new JoystickButton(fairBoard, 4);
    DoubleSupplier fairJoystickY = ()-> fairBoard.getRawAxis(1);
    DoubleSupplier fairJoystickX = ()-> fairBoard.getRawAxis(0);
    ...
    //FAIR:
    Command fairDrive = new DriveFair(driveSubsystem, 
    visionSubsystem, 
      () -> Util.modifyAxis(fairJoystickX, 2),
      () -> !driverRightBumper.getAsBoolean(),
      () -> driverLeftBumper.getAsBoolean());
  
    Command fairPivot = new ManualPivot(pivotSubsystem, ()->Util.modifyAxis(fairJoystickY, 2));

    driveSubsystem.setDefaultCommand(fairDrive);
    pivotSubsystem.setDefaultCommand(fairPivot);
    ...
    // driverRightTrigger.whileTrue(new IntakeFromShooter(shooterSubsystem));
    driverRightTrigger.whileTrue(new TowerIntake(intakeSubsystem, pivotSubsystem, shooterSubsystem, true, ledSubsystem, this::intakeCallback));
    fairShootButton.whileTrue(new ShootFair(driveSubsystem, shooterSubsystem, pivotSubsystem, visionSubsystem, ledSubsystem));
